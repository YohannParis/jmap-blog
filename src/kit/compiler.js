...
class KitCompiler {
	 ...
	 * @returns {Promise<Object>} - Result of compilation
	 */
	async recursivelyCompileKitFile(filePath, variablesDict = {}, forbiddenImportPaths = []) {
		const result = {
			successful: false,
			resultMessage: "",
			compiledCode: "",
		...
		if (forbiddenImportPaths.some((p) => p.toLowerCase() === filePath.toLowerCase())) {
		...
		try {
			...
			for (let currentComp = 0; currentComp < comps.length; currentComp++) {
				...
				const isSpecialComment = this.isSpecialComment(compString, comps, currentComp);
				...
				while (specialCommentComp < comps.length) {
					...
					if (commentEndIndex !== -1) {
						...
						if (commentEndIndex + 3 < commentCompString.length) {
							const possibleSuffix = commentCompString.substring(commentEndIndex + 3);
				...
				const { keyword, predicate } = this.parseSpecialComment(specialCommentString);
				...
				if (keyword.toLowerCase() === "@import-partial") {
					...
					for (const importString of imports) {
						const cleanedImportString = this.pruneCommonImportSyntaxCharacters(importString.trim());
						let fullImportFilePath = this.resolveRelativePath(cleanedImportString, filePath);
						...
						if (extension === ".kit") {
							...
							const importResult = await this.recursivelyCompileKitFile(pathInfo.path, variablesDict, newForbiddenPaths);
	...
	parseSpecialComment(commentString) {
		...
		const commentContent = commentString.substring(commentString.indexOf("<!--") + 4, commentString.lastIndexOf("-->")).trim();

		// Find the key character ($ or @)
		const keyCharIndex = Math.min(commentContent.indexOf("$") === -1 ? Infinity : commentContent.indexOf("$"), commentContent.indexOf("@") === -1 ? Infinity : commentContent.indexOf("@"));
		...
		if (keywordEnd < commentContent.length) {
			const predicateStart = commentContent.substring(keywordEnd).search(/[^\s=:]+/);

			if (predicateStart !== -1) {
				let predicateContent = commentContent.substring(keywordEnd + predicateStart);
